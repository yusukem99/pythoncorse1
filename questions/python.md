...
研究者は集めたたくさんの論文の中から、特定の言葉を含むものだけを抽出したいと思いました。

`doc_list`に論文の内容のリスト、`keyword`に検索する言葉が渡されます。

検索する言葉`keyword`が含まれる論文のインデックスのリストを返すように、以下の`word_search`関数を完成させてください。
...
研究者は先に作った関数`word_search`を複数の文字で検索できるようにしたいと思いました。

`multi_word_search()`関数は、引数`keywords`で検索する複数の言葉がリストで渡されます。

それぞれのキーワードを**キー**、論文のインデックスのリストを**バリュー**としたペアの辞書型を返すように関数を完成させてください。

問２で作成した`word_search`を活用してください。
...
下のグラフは$200から始めて500回スロットを回した時の手持ち金の推移です。
...
マリオカートで優勝するために最も有効なアイテムを見つけるためにデータセットを分析しています。
...
リスト内の要素がそれぞれ走者を表し、`items`は獲得したアイテムのリスト、`finish`がそのレースでの順位を表します。

`best_items()`はこのようなリストを引数で受け、一位の走者が獲得したアイテムをカウントしたものを辞書型で返します。

以下の小さなデータで実験してみると正しい結果が得られました。

フルデータセットで実行してみると`TypeError`でクラッシュしてしまいました。原因を調査してコードが動作するように修正してください。

# 問３
前回の授業でトランプのブラックジャックについて扱いました。

もし下の例のように、手札の合計を表す型`BlackjackHand`があり、比較演算子をオーバーロードしてその大小を比較出来たら便利ですね。
