...
`a-e`のそれぞれの文字列の長さを`length`に代入してください。ここで`len()`関数を使用しないこと。

## 問１

データサイエンティストの仕事の80%はデータクレンジングで、残りの20%はその作業に不満をいうことだと言われています。

ここではZIPコードのデータクレンジングをする関数を作ってみます。

与えられた`zip_code`が正しいフォーマットか判定し`True`または`False`で返す関数`is_valid_zip`を完成させてください。

ZIPコードとはアメリカ合衆国の郵便番号で、そのフォーマットは５桁の数字です。

ヒント：str型は便利なメソッドを備えています。`help(str)`を使ってみてください。
...
`doc_list`に論文の内容のリスト、`keyword`に検索する言葉が渡されます。検索する言葉`keyword`が含まれる論文のインデックスのリストを返すように、以下の`word_search`関数を完成させてください。
...
それぞれのキーワードを**キー**、含まれる論文のインデックスのリストを**バリュー**としたペアの辞書型を返すように関数を完成させてください。
