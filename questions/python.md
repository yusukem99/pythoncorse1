...
このスロットマシーンは長期的見ると利益がでることが分かったので、今回は`matplotlib`を使用してグラフを描いてみようと思います。

下のグラフは$200から始めて500回スロットを回した時の手持ち金の推移です。

しかしこのグラフは未完成で、見る人の誤解を招きます。以下の変更を加えてよりよいグラフにしてください。
...
そのメソッドの使い方が分からなければ`help()`関数を使いましょう。
...
リスト内の要素がそれぞれ一人の走者を表し、`items`は獲得したアイテムのリスト、`finish`がその走者の順位を表します。

`best_items()`関数はこのようなリストを引数に受け、一位の走者が獲得したアイテムをカウントしたものを辞書型で返します。

上のデータを用いて`best_items()`関数を実行してみると正しい結果が得られました。
